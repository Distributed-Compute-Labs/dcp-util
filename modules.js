...
app.post('*', (request, result, next) => {
  if (debug) { console.log('Processing request ' + request.url + ' in debug mode.') }
      ...
      return database.read(moduleLocation, false, false).then(moduleString => {
        ...
        for (let i = 0; i < moduleLines.length; i++) {
          ...
          if (posS === -1) {
            ...
            if (posS !== -1) { moduleLines[i] = moduleLines[i].slice(0, posS) }
            continue
          }
        ...
        moduleLines.push(';')
        if (debug) { loadedPackages[packageName][filePath] = moduleString.toString() } else { loadedPackages[packageName][filePath] = moduleLines.join('\n') }
        ...
        for (let posS = -1, posE = -1, i = 0; i < moduleLines.length && (posS === -1 || posE !== -1); i++) {
          if (posS === -1) {
            ...
            posE = moduleLines[i].indexOf(']')
            if (posE === -1) { depList += moduleLines[i] } else { depList += moduleLines[i].slice(0, posE - 1) }
          }
        }
...
var deployModuleVersion = function (packageJSON, message) {
  return new Promise((resolve, reject) => {
    ...
    for (let filename in message.files) {

...
for address in "${jobIds[@]}"; do
  if [[ "$?" -ne 0 && $utilOutput != *"${address}"* ]]; then
    echo "cancelJob operation did not cancel job with address: $address"
    exit 1
  fi
done
...
if [[ ${#statuses[@]} -ne 5 ]]; then
  echo "Expected 5 cancellations, got ${#statuses[@]}."
  exit 1
fi

for status in "${statuses[@]}"; do
  if [[ $status != "cancelled" ]]; then
    echo "One of the jobs did not have status cancelled"
    exit 1
  fi
done
